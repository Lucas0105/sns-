...
const JoinAgree = ({state, dispatch})=>{

    const history = useHistory();


    const textarea1 = `제1조 (목적)
    ...
    const textarea2 = `1. 개인정보 수집목적 및 이용목적
    ...
    const [bAgreeAll, bAgreeAllSet] = useState(false);
    const [bAgree1, bAgree1Set] = useState(false);
    const [bAgree2, bAgree2Set] = useState(false);
    const [bAgree3, bAgree3Set] = useState(false);
    const [bAgree4, bAgree4Set] = useState(false);

    function bChecked(bAgree){

        if(bAgreeAll === true || bAgree ===true){
            bAgree1Set(false);
            bAgree2Set(false);
            bAgree3Set(false);
            bAgree4Set(false);
    ...
    useEffect(()=>{
        if((bAgree1 && bAgree2 && bAgree3 && bAgree4) ===false){
            bAgreeAllSet(false);
        }
        else if ((bAgree1 === true && bAgree2 === true && bAgree3 === true && bAgree4 === true )){
            bAgreeAllSet(true);
        }
    },[bAgree1, bAgree2, bAgree3, bAgree4])
    ...
    return(
        <div className="JoinAgree">
            ...
            <div className="registContainer">
                <input name="agreeAll" type="checkBox"  onChange={()=>bChecked()} checked={bAgreeAll}/>
                <label htmlFor="agreeAll">이용약관, 개인정보 수집 및 이용에 모두 동의합니다.</label><br/>

                <input name="agree1" type="checkBox" 
                onChange={()=>{bAgree1 ? bAgree1Set(false):bAgree1Set(true)}} checked = {bAgree1} />
                <label htmlFor="agree1">이용약관 동의(필수)</label><br/>
                <div className="textArea">
                    <textarea defaultValue={textarea1} readOnly></textarea><br/>
                </div><br/>
                <input name="agree2" type="checkBox"
                onChange={()=>{bAgree2 ? bAgree2Set(false):bAgree2Set(true)}} checked = {bAgree2}/>
                <label htmlFor="agree2">개인정보 수집 및 이용(필수)</label><br/>
                <div className="textArea">
                    <textarea defaultValue={textarea2} readOnly></textarea>
                </div><br/>
                <div className="mText">
                    <p> 마케팅 활용 동의 및 광고 수신 동의</p>
                    <p> 서비스와 관련된 신상품 소식, 이벤트 안내,<br/> 고객 혜택 등 다양한 정보를 제공합니다.</p>
                </div>
                    <br/>
                    <br/>
                <input name="agree3" type="checkBox" 
                onChange={()=>{bAgree3 ? bAgree3Set(false):bAgree3Set(true)}} checked = {bAgree3}/>
                <label htmlFor="agree3">SMS 수신 동의(선택)</label><br/>

                <input name="agree4" type="checkBox"
                onChange={()=>{bAgree4 ? bAgree4Set(false):bAgree4Set(true)}} checked = {bAgree4}/>
                <label htmlFor="agree4">위치 정보 수집 동의(선택)</label><br/>
                ...
                <div className="regist">
                    <button onClick={()=>{
                        if(bAgree1 && bAgree2 === true){
                            dispatch({type:'joinAgree', agree1:bAgree1, agree2:bAgree2, agree3:bAgree3, agree4:bAgree4});
                            history.push("/join");
                        }
                        else{
                            alert('필수 항목에 체크해주세요.');
...
export default connect(connectToStore)(JoinAgree)